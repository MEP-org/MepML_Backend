...
def singin(request):
    ...
    try:
        user = User.objects.get(firebase_uuid=firebase_id)
    except:
        print("No such User")
    ...
    try:
        person = Student.objects.get(user__firebase_uuid=firebase_id)
        LoginUserSerializer.Meta.model = Student
    except:
        print("It's not a student")  
    try:
        person = Professor.objects.get(user__firebase_uuid=firebase_id)
